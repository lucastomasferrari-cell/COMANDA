<script lang="ts" setup>
  ...
  const { t } = useI18n()
  const { orderTypeDistribution } = useDashboard()
  ...
  const palette = ['#f57c00', '#0f766e', '#1d4ed8', '#e11d48', '#7c3aed', '#d97706']
  ...
  async function loadData () {
    try {
      loading.value = true
      const response = await orderTypeDistribution(filter.value)
  ...
  <VCard class="dashboard-panel" height="380">
    <VCardTitle class="dashboard-panel__header">
      <div class="dashboard-panel__title">
        <VIcon class="dashboard-panel__icon" icon="tabler-chart-pie" size="22" />
        {{ t('dashboard::dashboards.order_type_distribution') }}
      </div>
      <VSelect
        v-model="filter"
