<script lang="ts" setup>
  ...
  const { t } = useI18n()
  const { paymentsOverview } = useDashboard()
  ...
  const palette = ['#0f766e', '#1d4ed8', '#f57c00', '#e11d48', '#7c3aed', '#d97706']
  ...
  async function loadData () {
    try {
      loading.value = true
      const response = await paymentsOverview(filter.value)
  ...
  <VCard class="dashboard-panel" height="380">
    <VCardTitle class="dashboard-panel__header">
      <div class="dashboard-panel__title">
        <VIcon class="dashboard-panel__icon" icon="tabler-credit-card" size="22" />
        {{ t('dashboard::dashboards.payments_overview') }}
      </div>
      <VSelect
        v-model="filter"
