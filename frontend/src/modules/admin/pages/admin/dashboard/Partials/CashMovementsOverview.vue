<script lang="ts" setup>
  ...
  const { t } = useI18n()
  const { cashMovementsOverview } = useDashboard()

  const loading = ref(false)
  const filter = ref('all_time')
  const chartRef = ref<HTMLCanvasElement | null>(null)
  let chartInstance: Chart | null = null
  const data = ref({ labels: [], data: [], colors: [], currency: 'JOD' })
  const palette = ['#d97706', '#0f766e', '#1d4ed8', '#e11d48', '#7c3aed', '#f57c00']
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            displayColors: false,
            callbacks: {
              label: (context: TooltipItem<'doughnut'>) => `${t('dashboard::dashboards.total_amount')}: ${formatPrice(context.raw as number, data.value.currency)}`,
            },
          },
        },
        cutout: '66%',
  ...
  async function loadData () {
    try {
      loading.value = true
      const response = await cashMovementsOverview(filter.value)
  ...
  <VCard class="dashboard-panel" height="380">
    <VCardTitle class="dashboard-panel__header">
      <div class="dashboard-panel__title">
        <VIcon class="dashboard-panel__icon" icon="tabler-cash" size="22" />
        {{ t('dashboard::dashboards.cash_movements_overview') }}
      </div>
      <VSelect
        v-model="filter"
