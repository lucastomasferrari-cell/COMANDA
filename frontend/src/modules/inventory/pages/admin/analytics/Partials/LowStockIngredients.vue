<script lang="ts" setup>
  ...
  const fetchData = async () => {
    loading.value = true
    try {
      const response = await http.get('/v1/inventories/analytics/low-stock-ingredients', {
...
<template>
  <VCard style="height: 400px">
    <VCardTitle class="border-b pb-2 mb-4 d-flex align-center gap-1 font-weight-bold text-h6">
      <VIcon color="primary" icon="tabler-alert-triangle" size="20" />
      {{ t('inventory::inventories.analytics.low_stock_ingredients') }}
    </VCardTitle>
    <VCardText
