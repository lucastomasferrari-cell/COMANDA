<script lang="ts" setup>
  ...
  import {
    useDiscount,
  } from '@/modules/promotion/composables/discount.ts'
  ...
  const { user } = useAuth()
  const { getFormMeta, update, store } = useDiscount()
  ...
  async function submit () {
    if (
      ...
      await router.push({ name: 'admin.discounts.index' } as unknown as RouteLocationRaw)
    }
  }
